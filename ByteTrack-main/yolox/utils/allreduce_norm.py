...
def all_reduce(py_dict, op="sum", group=None):
    ...
    flatten_tensor =  mindspore.ops.Concat([py_dict[k].flatten() for k in py_key])
    ...
    split_tensors = [
        x.reshape(shape)
        for x, shape in zip(mindspore.ops.Split(flatten_tensor, tensor_numels), tensor_shapes)
    ]
