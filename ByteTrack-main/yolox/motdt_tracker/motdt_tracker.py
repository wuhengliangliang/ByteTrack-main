...
class OnlineTracker(object):
    ...
    def update(self, output_results, img_info, img_size, img_file_name):
        ...
        if len(detections) > 0:
            ...
            mindspore.tensor.from_numpy(rois),
            mindspore.tensor.from_numpy(scores.reshape(-1)).to(mindspore.tensor.from_numpy(rois).dtype),
            mindspore.ops.ZerosLike(mindspore.tensor.from_numpy(scores.reshape(-1))),
            0.7,
            )
    ...
    def _xyxy_to_tlwh_array(bbox_xyxy):
        ...
        elif isinstance(bbox_xyxy, mindspore.Tensor):
