...
def load_net(fname, net, prefix='', load_state_dict=False):
    ...
    with h5py.File(fname, mode='r') as h5f:
        ...
        for k, v in net.state_dict().items():
            k = prefix + k
            if k in h5f:
                param = mindspore.tensor.from_numpy(np.asarray(h5f[k]))
...
class Model(nn.Cell):
    ...
    def construct(self, x):
        ...
        att_weights = mindspore.nn.Sigmoid(self.conv_att(feature))

        linear_feautres = []
        for i in range(self.n_parts):
            masked_feature = feature * mindspore.ops.ExpandDims(att_weights[:, i], 1)
            pooled_feature = mindspore.ops.AvgPool(masked_feature, masked_feature.size()[2:4])
            linear_feautres.append(
                getattr(self, 'linear_feature{}'.format(i+1))(pooled_feature.view(pooled_feature.size(0), -1))
            )
        op = mindspore.ops.Concat(axis=1)
        concat_features = op(linear_feautres)
        normed_feature = concat_features / mindspore.ops.clip_by_value(mindspore.nn.Norm(concat_features, 2, 1, keepdim=True), min=1e-6)
...
def extract_reid_features(reid_model, image, tlbrs):
    ...
    with mindspore.ops.stop_gradient():
        im_var = Variable(mindspore.tensor.from_numpy(patches))
        im_var = im_var.cuda()
